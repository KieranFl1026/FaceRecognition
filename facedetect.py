...
def detect_and_draw(img, cascade, detected):
    ...
    gray = cv.imwrite((img.width,img.height), 8, 1)
    image_scale = img.width / smallwidth

    small_img = cv.imwrite((cv.Round(img.width / image_scale), cv.Round (img.height / image_scale)), 8, 1)
...
if __name__ == '__main__':
    ...
    cascade = cv.imread(options.cascade)
